...
6. **Provenance.** Every Log entry and every Status claim traces to a journal date, a Slack permalink, a Jira key, or a conversation with Royce. Say when something is from the vault alone and not yet reconciled. A future-dated item in a journal is a plan as of that day, not a fact: write it as "planned (per [[date]])" and check newer sources before promoting it to a milestone or a Status claim.
